...
## This Layer Uses 000 For The Overview Then 001 002 003 For Additional Index References
The overview registry lives at `000.INDEX.*-Ref.md`. Additional index-level references continue in numeric order as `001.INDEX.*-Ref.md`, `002.INDEX.*-Ref.md`, and so on, so the index band has room to grow without colliding with ordinary branch numbering.
