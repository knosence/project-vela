...
- `vela merge apply-follow-up <target> --reason ...` creates the canonical SoT and repoints the prior owners through governed mutation.
...
- `/api/n8n/merge/follow-ups/apply` executes an approved merge follow up.
...
## Local Continuous Deployment Uses User Services

- `ops/bootstrap/install-user-services.sh` installs the user-level `systemd` units.
- `vela-api.service` keeps the governed API available locally.
- `vela-patrol.timer` schedules patrol through the CLI entrypoint with `--actor system`.
- `vela-night-cycle.timer` schedules the night cycle through the CLI entrypoint with `--actor system`.
- See [[240k.DOMAIN.Local-Service-Deploy-Ref]] for the canonical install and enable path.

## Telegram Delivery Is Optional And Governed

- `vela telegram status` reports whether Telegram is enabled and sufficiently configured.
- `vela telegram test-send` sends a governed test message through the configured bot and chat.
- `/api/n8n/telegram/status` exposes Telegram readiness through the service layer.
- `/api/n8n/telegram/test-send` sends a governed Telegram test message through the service layer.
- When Telegram is enabled, the night cycle may send the DC report and blocked summary through the configured bot.
- Telegram delivery does not replace the canonical matrix artifacts. It only mirrors them outward for operator awareness.
