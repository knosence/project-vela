...
## This Constitution Establishes the Human Root, Registry, Sovereign Surfaces, and Trinity
The single cornerstone is [Cornerstone.Knosence-SoT.md](/home/knosence/vela/knowledge/Cornerstone.Knosence-SoT.md). The top-level registry is [000.INDEX.Knosence-Matrix-Ref.md](/home/knosence/vela/knowledge/000.INDEX.Knosence-Matrix-Ref.md). The Trinity consists of the Cornerstone, the Key at [230.DOMAIN.SoT-Source-of-Truth-SoT.md](/home/knosence/vela/knowledge/230.DOMAIN.SoT-Source-of-Truth-SoT.md), and the System at [240.DOMAIN.Matrix-SoT.md](/home/knosence/vela/knowledge/240.DOMAIN.Matrix-SoT.md). Sovereign surfaces include the cornerstone, identity SoTs, the canonical watchlist SoT, and local matrix laws.

## This Constitution Keeps Canonical Matrix Files Flat and Moves Operational Exhaust Elsewhere
Canonical SoTs and canonical refs live directly under [knowledge](/home/knosence/vela/knowledge). Non-canonical artifacts such as packets, proposals, logs, archives, and templates live under [ARTIFACTS](/home/knosence/vela/knowledge/ARTIFACTS), and intake lands in [INBOX](/home/knosence/vela/knowledge/INBOX).
...
Ordinary SoTs should stay content-dense until the growth ladder actually warrants abstraction.
...
## This Constitution Applies the Numbered Naming Scheme to Canonical Files and References
Canonical SoTs follow `{ID}.{Context}.{Subject}-SoT.md` for hubs and branch SoTs, with the cornerstone as the singular exception. The index band uses `000.INDEX.*-Ref.md`, then `001.INDEX.*-Ref.md`, `002.INDEX.*-Ref.md`, and so on. Other canonical refs use parent-based letter-suffixed IDs such as `230a`, `240a`, or `411a`. Direct children of a hub use `XX0`, grandchildren use `XXX`, and promoted descendants return to the next local `XX0` under the governing hub. Retired IDs may be reused only when the old file no longer exists.
