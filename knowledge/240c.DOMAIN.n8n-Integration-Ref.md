...
`GET /api/n8n/merge/candidates` lists repeated-ref merge candidates detected through the `3+ entity` rule. `GET /api/n8n/merge/proposals` lists governed merge proposals already opened by the night cycle. These surfaces expose visibility only. They do not create or approve merges directly.
