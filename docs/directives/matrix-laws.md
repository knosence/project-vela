...
## Law 3 Every Active Source of Truth Shall Be Visible in the Matrix Index
If a SoT is active but absent from the index layer, it is operationally invisible and governance degrades. The index is the top-level view of the matrix.

## Law 4 Content Shall Live Once and Be Seen Through Pointers
Canonical content belongs in one home. Every other place that needs awareness of that content should point rather than duplicate.
...
Non-cornerstone SoTs shall use numbered dimension-prefixed filenames such as `100.WHO.Circle-SoT.md` for hubs and `220.WHAT.Repo-Watchlist-SoT.md` for child branch SoTs. The cornerstone remains the sole non-numbered top-level exception.
